...
bool SQLiTest::errorBasedTest(CHttpClient* pHttpClient, Item *pItem, unsigned pos, long &averageTime)
{
	...
	for (unsigned i = 0; i < m_vecEBTestCase.size(); i++)
	{
		...
		if (resultState != 0)
		{
			...
			WriteFile("网址树_resultId" + to_string(pResult->id), html);
			break;
		}
	}
...
bool SQLiTest::boolBasedTest(CHttpClient* pHttpClient, Item *pItem, unsigned pos, long &averageTime)
{
	...
	if (resultState > 0)
	{
		...
		if (resultState == 201)
		{
			WriteFile("网址树_resultId" + to_string(pResult->id) + "ori", to_string(oriHtml.length()) + "\r\n" + oriHtml);
		}
		else{
			WriteFile("网址树_resultId" + to_string(pResult->id) + "identiRight", to_string(rightIdentiHtml.length()) + "\r\n" + rightIdentiHtml);
			WriteFile("网址树_resultId" + to_string(pResult->id) + "identiError", to_string(errorIdentiHtml.length()) + "\r\n" + errorIdentiHtml);
		}
		WriteFile("网址树_resultId" + to_string(pResult->id) + "Right", to_string(rightHtml.length()) + "\r\n" + rightHtml);
		WriteFile("网址树_resultId" + to_string(pResult->id) + "Error", to_string(errorHtml.length()) + "\r\n" + errorHtml);
}
